...
import { Container, Row, Col, Card, Button, Modal, Toast} from 'react-bootstrap';
...
class OrderHistory extends Component {
    ...
    async handleOpenModal(order){
        var requestOptions = {
          ...
          if (res !== undefined && res.status === 200){
            ...
            this.setState({ showToast: true, toastMessage: 'Ocorreu um erro ao carregar o histório de compras', toastType: 'danger' });
          }
    ...
    render() {
      const totalPrice = this.state.orderItems.reduce((acc, item) => acc + item.totalPrice, 0);
        return (
            <>
                ...
                {this.state.orders.map(order => (
                  <Col key={order.id} sm={4} className="mb-4">
                    <Card style={{width:"400px"}}>
                        ...
                        <Card.Text>Data: {new Date(order.date).toLocaleDateString()}</Card.Text>
                        <Card.Text>Hora: {new Date(order.date).toLocaleTimeString()}</Card.Text>
                        <Button variant="primary" onClick={() => this.handleOpenModal(order)}>Ver Detalhes</Button>
                        </Card.Body>
                    </Card>
                    </Col>
                ))}
          ...
          {this.state.selectedOrder && (
              ...
              <Card.Text>Data: {new Date(this.state.selectedOrder.date).toLocaleDateString()}</Card.Text>
                <Card.Text>Hora: {new Date(this.state.selectedOrder.date).toLocaleTimeString()}</Card.Text>
              <p key={999}>
